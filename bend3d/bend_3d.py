...
alphabar      = 1e4 * mu / (1 / 5**2)    # alpha_max in the original dimensional scaling
q             = Constant(0.01)          # continuing parameter for SIMP interpolation
maxh          = 0.1                   # maximum mesh size
volfrac       = 0.05            # fluid volume fraction
...
# distanza radiale dal centro dell'inlet, nel piano y-z (faccia x=0)
rho1_sq = (y - 1/2)**2 + (z - 4*l)**2
val1 = gbar * (1 - rho1_sq / r_pipe**2)   # componente x della velocità, positiva = entra nel dominio

# distanza radiale dal centro dell'outlet, nel piano x-y (faccia z=0)
rho2_sq = (x - 4*l)**2 + (y - 1/2)**2
val2 = -gbar * (1 - rho2_sq / r_pipe**2)  # componente z della velocità, negativa = esce dal dominio
...
Fp = Res - inner(p/gamma, q_test)*dx  + inner(div(u)*gamma, div(v))*dx#(degree=10)  #preconditioned residual
...
if __name__ == "__main__":
    ...
    if float(Re) > 5:
        PETSc.Sys.Print(f"Using continuing strategy to reach Re={float(Re)}, with q_0={q_0}.")
        Re_final = int(float(Re))
        Re_v = [1,10, 100] + list(range(200, Re_final + 1, 500))
        if Re_v[-1] != Re_final:
